...
execute if score &game_running hotpotato.root matches 1 if score &player_particles hotpotato.root matches 1 as @a[tag=hotpotato.has_potato] run function hotpotato:app/game/particles

execute if score &speed_before_explosion_counter hotpotato.root matches 0.. run scoreboard players remove &speed_before_explosion_counter hotpotato.root 1
execute if score &speed_before_explosion_counter hotpotato.root matches 0 run effect give @a[tag=hotpotato.has_potato] minecraft:speed infinite 1 true